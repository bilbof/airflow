    ...
    def _get_task_instances(
        ...
        *,
        ...
        if task_ids is not None:
            tis = tis.filter(
                or_(
                    TI.task_id.in_(t for t in task_ids if isinstance(t, str)),
                    tuple_(TI.task_id, TI.map_index).in_(t for t in task_ids if not isinstance(t, str)),
                ),
            )
        ...
        elif exclude_task_ids is not None:
            tis = tis.filter(
                TI.task_id.notin_(t for t in exclude_task_ids if isinstance(t, str)),
                tuple_(TI.task_id, TI.map_index).notin_(
                    t for t in exclude_task_ids if not isinstance(t, str)
                ),
            )
